...
from dqn import NoisyDuelingDQN as DQN
...
class Agent:
    ...
    def run(self, is_training=True, render=False, resume=False):
        ...
        for episode in itertools.count():
            ...
            while not terminated and episode_reward < self.stop_on_reward:
                # doesn't use epsilon greedy policy since dqn has noisy network
                # instead, reset noise before action selection
    ...
    def optimize(self, mini_batch_data, policy_dqn, target_dqn, memory):
        ...
        # switch to evaluation mode to avoid noisy selection
        policy_dqn.eval()
        with torch.no_grad():
